 ...
 *   
 *     'location': '...',       // STRING  Location of the meter UI, one of 't'op, 'b'ottom, 'l'eft, or 'r'ight. NOTE that 't'op is not very reliably placed, and could overflow into your field.
...
;(function($) {
  ...
  var audit = function(o) {
    // requirements
    if (!o.requirements) {
      o.requirements = $.fn.simplePassMeter.defaults.requirements;
    } else {
      for (var k in o.requirements) {
        if (!$.fn.simplePassMeter.defaults.requirements[k]) {
          // not in our defaults, does it have everything we need?
          if (typeof o.requirements[k].value == 'undefined' ||
              typeof o.requirements[k].message != 'string' ||
              (typeof o.requirements[k].regex != 'string' &&
               !$.isFunction(o.requirements[k].callback))) {
            o.requirements[k] = null;
          } else {
        ...
        if (typeof o.requirements[k].value == 'undefined') {
          o.requirements[k].value = $.fn.simplePassMeter.defaults.requirements[k].value;
        }
        if (typeof o.requirements[k].message != 'string') {
          o.requirements[k].message = $.fn.simplePassMeter.defaults.requirements[k].message;
        }
        if (typeof o.requirements[k].regex != 'string' &&
            $.fn.simplePassMeter.defaults.requirements[k].regex) {
          o.requirements[k].regex = $.fn.simplePassMeter.defaults.requirements[k].regex;
        }
        if (!$.isFunction(o.requirements[k].callback) &&
            $.fn.simplePassMeter.defaults.requirements[k].callback) {
          o.requirements[k].callback = $.fn.simplePassMeter.defaults.requirements[k].callback;
        }
        
        // key-specific checks
        if (k == 'minLength') {
          if (!Number(o.requirements[k].value) || o.requirements[k].value < 1) {
            o.requirements[k].value = $.fn.simplePassMeter.defaults.requirements[k].value;
          }
        }
      }
    ...
    if (!o.ratings || !o.ratings.length) {
      o.ratings = $.fn.simplePassMeter.defaults.ratings;
    } else {
      var ps = 0;
